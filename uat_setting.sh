...
if [ -z "$COMPOSE_PROJECT_NAME" ] || [ -z "$WP_URL" ]; then
    echo "未定義必要的變數: COMPOSE_PROJECT_NAME, WP_URL。請檢查 .env 檔案。"
    exit 1
fi

UAT_DOMAIN=$(echo "$WP_URL" | sed 's/.*\/\///; s/\/.*//')
if [ -z "$UAT_DOMAIN" ]; then
    echo "無法取得Domain設定。請檢查 .env 檔案。"
    exit 1
fi

echo "UAT_DOMAIN = $UAT_DOMAIN"
...
# 建立nginx設定檔
echo "********** 建立nginx設定檔 **********"
sudo sh deployment_scripts/nginx_setting.sh $COMPOSE_PROJECT_NAME $UAT_DOMAIN

# 修改wp.sql中的連結
echo "********** 修改wp.sql中的連結設定 **********"
sudo sh deployment_scripts/repleace_db_url.sh db/uat/wp.sql $WP_URL
