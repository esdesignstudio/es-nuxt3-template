...
class MessageFormatter
{
     ...
     * @return string
     */
    public function format(\WPMailSMTP\Vendor\Psr\Http\Message\RequestInterface $request, \WPMailSMTP\Vendor\Psr\Http\Message\ResponseInterface $response = null, \Exception $error = null)
    {
        $cache = [];
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \WPMailSMTP\Vendor\GuzzleHttp\Psr7\str($request);
                    break;
                case 'response':
                    $result = $response ? \WPMailSMTP\Vendor\GuzzleHttp\Psr7\str($response) : '';
                    break;
                case 'req_headers':
     ...
     * @return string
     */
    private function headers(\WPMailSMTP\Vendor\Psr\Http\Message\MessageInterface $message)
    {
        $result = '';
